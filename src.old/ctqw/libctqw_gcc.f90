...
!	use IFPORT
	...
	subroutine qw_cheby(psi,psiT,dt,H,Emin,Emax,N)
		...
		U = bessel_jn(0,alpha)*phi0 + 2.d0*ii*bessel_jn(1,alpha)*phi1

		terms = 0
		do while (abs(2.d0*bessel_jn(terms,alpha)) > 1.d-18)
			terms = terms + 1
		end do
!
...
!			call progressbar(m,terms)
			...
			U = U + 2.d0*(ii**m)*bessel_jn(m,alpha)*phi2

			write(26,"(E24.15E3)")abs(2.d0*bessel_jn(m,alpha))
